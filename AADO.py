...
def updSAADO1(i,j,k,dC,nA,S,nS,M,nM,nD,r):
    d = S[i+k-1,j+k-1]
    if(dC.ndim != 1):
        for l in range(dC.shape[1]):
            d += (dC[i+k-1,l] - dC[j+k-1,l])**2
    else:
        d += (dC[i+k-1] - dC[j+k-1])**2
    if(d < 1e-15):
        d = 0
    nS[i,j] = d
    nS[j,i] = d
...
def updSAADO2(i,j,k,k2,dC,nA,S,nS,M,nM,nD,r):
    d = S[i+k-1,j+k2-1]
    for l in range(dC.shape[1]):
...
def updSAADO3(i,j,p,k,dC,nA,S,nS,M,nM,nD,r):
    d = S[i+k-1,j+p]
    for l in range(dC.shape[1]):
...
def updSAADO4(i,j,p,dC,nA,S,nS,M,nM,nD,r):
    d = S[i+p,j+p]
    for l in range(dC.shape[1]):
...
'''
    ...
    r: radius
'''
def AADO(U,dU,dC,label,A,S,M,D,r):
    nA,nS,nM,nD = InitAADO(U,dU,label)
    for k in range(1,dU.shape[0]):
        for i in range(dU[k-1]-k+2,dU[k]-k+1):
            ...
            for j in range(i,dU[k]-k+1):
                updSAADO1(i,j,k,dC,nA,S,nS,M,nM,nD,r)
            for k2 in range(k+1,dU.shape[0]):
                for j in range(dU[k2-1]-k2+2,dU[k2]-k2+1):
                    updSAADO2(i,j,k,k2,dC,nA,S,nS,M,nM,nD,r)
            for j in range(dU[-1]-dU.shape[0]+2,U.shape[0]-dU.shape[0]+1):
                updSAADO3(i,j,dU.shape[0]-1,k,dC,nA,S,nS,M,nM,nD,r)
    for i in range(dU[-1]-dU.shape[0]+2,U.shape[0]-dU.shape[0]+1):
        ...
        for j in range(i,U.shape[0]-dU.shape[0]+1):
            updSAADO4(i,j,dU.shape[0]-1,dC,nA,S,nS,M,nM,nD,r)
